...
description: Complete rubric specifications for all 8 BIT320 labs (1.1-5.1). Use these templates to manually create/update rubrics in Canvas UI. Follows Code Quality Scoring Progression (SKILL v3.3).
---

# BIT320 Complete Rubric Specifications

**How to use this document:**
1. For each lab, copy the rubric criteria and point values
2. Create/edit rubric in Canvas UI
3. Add each criterion with the specified point values and rating descriptions
4. Ensure "Total Points Possible" matches the assignment points
...
### Criterion 2: Code Fundamentals - Functions, Loops & Error Handling (8 pts, 32%) **[NEW]**
...
**Status:** CREATE NEW RUBRIC

**Criteria (5 total = 100 pts):**
...
### Criterion 1: Well Formatted Document (4 pts, 16%)
- **Excellent (4):** Identification, organized sections. Screenshots of scheduling configuration and logs clearly labeled. Setup instructions clear and reproducible.
...
- **Excellent (5):** Script from Lab 4.1 reused cleanly. Error handling from Lab 4.1 still in place. Comments explain integration point for scheduler. Evidence script runs clean when invoked by scheduler.
...
1. **Reusable rubrics:** Labs 1.1, 1.2, and 4.2 are small and specific; mark as "Reusable: No" since they're assignment-unique.
2. **For instructors creating these in Canvas UI:** Copy the criterion name, max points, and all three rating level descriptions exactly as written.
3. **Code Fundamentals progression:** Increases from Weeks 1-2 (setup) → Week 3-4 (error handling emphasis) → Week 5 (logging/testing emphasis). Update descriptions per week as noted.
4. **Grading:** Use these rubrics with the Code Quality Checklist reference for consistent scoring across all 5 weeks.
