...
- BIT320 (Lab 5.1 Redesign — Final Project with Student Choice, 2026-09-16): Restructured Lab 5.1 from single Windows Event Log Analysis task to three equal student-choice options (100 pts each, shared rubric). Options: (A) Service Health Monitoring & Auto-Remediation (proactive operations), (B) Configuration Management & Compliance Auditing (infrastructure-as-code), (C) Windows Event Log Analysis (reactive analytics—original task retained as option). All three graded on same rubric (Script Quality, Analysis/Reporting, Tool Selection, Documentation at 25 pts each). Created three comprehensive published resource pages (Option A, B, C) with detailed guidance, code examples, getting started sections, and "what excellent looks like." Updated Lab 5.1 assignment description to present options and link to resource pages. Reinforces course theme: "choose the right tool for the task." Students choose based on interest/career focus. Canvas API verified working (reconnected after app restart). courses/BIT320.md updated.
